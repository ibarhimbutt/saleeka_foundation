...
export default async function GetInvolvedPage({ searchParams }: { searchParams?: Promise<{ tab?: string }> }) {
      ...
      <section className="container">
        <Tabs defaultValue={initialTab} className="w-full">
          <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 gap-2 mb-8">
            {involvementTypes.map((item) => (
              <TabsTrigger key={item.id} value={item.id} className="text-xs sm:text-sm">{item.title}</TabsTrigger>
            ))}
          </TabsList>
          <TabsContent value="students"><StudentForm /></TabsContent>
          <TabsContent value="professionals"><ProfessionalForm /></TabsContent>
